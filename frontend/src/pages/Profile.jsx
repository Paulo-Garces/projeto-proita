...
export default function Profile() {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          setProfessional({
            ...
            planType: profile.user?.planType || profile.planType || null,
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: `Perfil de ${professional.name} no proITA`,
      text: `Confira o perfil profissional de ${professional.name} no proITA - O Guia dos Três Climas!`,
    ...
    <div className="bg-slate-50 min-h-screen pb-20 pt-20 md:pt-24">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 md:-mt-24 relative z-10">
        <div className="bg-white rounded-3xl shadow-xl p-6 md:p-10 border border-slate-100">
          ...
          <div className="flex flex-col gap-6 w-full border-b border-slate-100 pb-8 relative">
            
            {/* Redes Sociais Desktop - Canto Superior Direito */}
            <div className="hidden md:flex absolute top-0 right-0 items-center gap-3">
              {renderSocialLinks()}
              {isOwner && (
                <button onClick={() => { setIsSocialModalOpen(true); setSocialError(''); }} className="p-2 bg-slate-100 hover:bg-primary hover:text-white text-slate-500 rounded-full transition-colors cursor-pointer flex items-center justify-center border border-slate-200" title="Editar Redes Sociais">
                  <Edit2 size={14} />
                </button>
              )}
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-end text-center md:text-left w-full">
              ...
              <div className="hidden md:flex flex-row items-center justify-end gap-4 ml-auto mb-2">
                {professional.phone && (
                  <button onClick={handlePhoneClick} className="w-12 h-12 flex items-center justify-center bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-full transition-colors shadow-sm active:scale-95 cursor-pointer" title="Ligar">
                    <Phone size={20}/>
                  </button>
                )}
                <button onClick={handleWhatsApp} className="w-14 h-14 flex items-center justify-center bg-[#25D366] hover:bg-[#1fb355] text-white rounded-full transition-all shadow-md shadow-green-200/40 hover:scale-105 active:scale-95 cursor-pointer" title="WhatsApp">
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-[28px] h-[28px]"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                </button>
                <button onClick={handleShare} className="w-12 h-12 flex items-center justify-center bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-full transition-colors shadow-sm active:scale-95 cursor-pointer" title="Compartilhar Perfil">
                  <Share2 size={20}/>
                </button>
              </div>
            </div>

            {/* Ações Rápidas - Unificadas com o AdCard (botões circulares) */}
            <div className="flex md:hidden flex-row items-center justify-center gap-6 w-full mt-6">
              {professional.phone && (
                <button 
                  onClick={handlePhoneClick}
                  className="w-12 h-12 shrink-0 flex items-center justify-center bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-full transition-colors shadow-sm active:scale-95 cursor-pointer"
                  title="Ligar"
                >
                  <Phone size={20} />
                </button>
              ...
              <button 
                ...
                className="w-14 h-14 shrink-0 flex items-center justify-center bg-[#25D366] hover:bg-[#1fb355] text-white rounded-full transition-all shadow-md shadow-green-200/40 hover:scale-105 active:scale-95 cursor-pointer"
                title="WhatsApp"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-[28px] h-[28px] shrink-0">
          ...
          {/* Redes Sociais */}
          <div className="flex md:hidden justify-center items-center gap-4 py-4 border-b border-slate-100">
            {renderSocialLinks()}
            {isOwner && (
              <button
                onClick={() => {
                ...
                type="button"
                className="p-2 bg-slate-100 hover:bg-primary hover:text-white text-slate-500 rounded-full transition-colors cursor-pointer flex items-center justify-center border border-slate-200"
                  ...
                  if (valid.length === 0) {
                    return (
                      <div className="bg-slate-50 rounded-3xl p-12 border border-slate-100 text-center flex flex-col items-center justify-center space-y-4">
                        <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-xs text-slate-400">
                          <Store size={28} />
                        </div>
                        <h3 className="text-lg font-bold text-slate-800">Sem Parceiros</h3>
                        <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
                          Este profissional ainda não adicionou parceiros locais.
                        </p>
                      </div>
                    );
                  }
                        ...
                        {valid.map((partner, index) => {
                          ...
                          const imageElement = (
                            <img
                              ...
                              className="w-24 sm:w-32 shrink-0 aspect-[3/4] object-cover rounded-lg border border-slate-100"
                            />
                          );
                          ...
                          return (
                            <div 
                              key={index} 
                              className="flex flex-row gap-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm mb-4"
                            >
                              ...
                              <div className="flex flex-col justify-center flex-1 min-w-0 gap-1 sm:gap-2">
                                {partner.isMainSponsor && (
                                  <span className="inline-flex items-center w-fit px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-150 shrink-0">
                                ...
                                {partner.name && (
                                  <h4 className="text-base sm:text-lg font-bold text-slate-800 truncate" title={partner.name}>
                                    {partner.name}
                                  </h4>
                                )}
                                ...
                                <div className="space-y-1">
                                  {partner.partnerAddress && (
                                    <div className="flex items-center gap-1.5 text-slate-600 text-xs sm:text-sm truncate" title={partner.partnerAddress}>
                                      <MapPin className="text-slate-450 shrink-0" size={14} />
                                      <span className="truncate">{partner.partnerAddress}</span>
                                    </div>
                                  )}

                                  {partner.partnerPhone && (
                                    <div className="flex items-center gap-1.5 text-slate-600 text-xs sm:text-sm truncate" title={partner.partnerPhone}>
                                      <Phone className="text-slate-450 shrink-0" size={14} />
                                      <span className="truncate">{partner.partnerPhone}</span>
                                    </div>
                                  )}
                                </div>

                                {/* Link 'Visitar parceiro' */}
                                {partnerLink && (
